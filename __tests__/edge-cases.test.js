...
padding.y = 10
    ...
    test("Filters out non-TOML files correctly", async () => {
      ...
      expect(themes.map((t) => t.path)).toEqual(
        expect.arrayContaining([
          expect.stringMatching(/theme1\.toml$/),
          expect.stringMatching(/theme5\.toml$/),
        ]),
      );
    });
  });
  ...
  describe('Function "applyTheme" - Error Handling', () => {
    test("Throws error when theme file does not exist", async () => {
      await expect(applyTheme("non-existent-theme.toml", "alacritty.toml"))
        .rejects.toThrow();
    });

    test("Throws error when config file does not exist", async () => {
      await expect(
        applyTheme("themes/monokai_pro.toml", "non-existent-config.toml"),
      ).rejects.toThrow();
    });
...
background = '0x111111'
  ...
  describe('Function "unslugify" - Boundary Conditions', () => {
    ...
    test("Handles string with multiple underscores", () => {
      expect(unslugify("theme___with____many_underscores.toml")).toBe(
        "Theme With Many Underscores",
      );
    });
...
background = '0x${i.toString(16).padStart(6, "0")}'
    ...
    test("Handles rapid successive theme applications", async () => {
      ...
      for (let i = 1; i <= 3; i++) {
        ...
        expect(config.colors.primary.background).toBe(
          `0x${i}${i}${i}${i}${i}${i}`,
        );
      }
    });
